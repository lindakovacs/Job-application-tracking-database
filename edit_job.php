...
<fieldset title="Pay rates">
	...
	&pound;<input type="number" name="day_rate" min='50' max='2000' value="$row[day_rate]">
	
	<label>Contract?</label>
	<input type='checkbox' name='contract' $contract>
	
	<label>Salary low</label>
	&pound;<input type="number" name="salary_low" min='10000' max='200000' value="$row[salary_low]">

	<label>Salary high</label>
	&pound;<input type="number" name="salary_high" min='10000' max='200000' value="$row[salary_high]">
</fieldset>

<fieldset tite="Agency">
	<legend>Contact details</legend>
	<label>Via</label>
	<input type="text" list="via" name="via" value="$row[via]"><datalist id="via"><option value="TotalJobs"><option value="JobSite"><option value="BCS"></datalist>	
...
<fieldset title="Summary" class="clearfix">
  ...
  <textarea name="summary" cols="80" rows="4">On $date I applied for '$row[role]' based in $location. Reference: $reference. The agency is $agency via '$row[via]'. The company is $company.</textarea>
	...
	<input type="reset" title="Reset edit" value="Reset edit" class="clearfix">
</form>

<form action="add_activity.php?applicationid=$id" method="post" id="met-form">
	<input type="submit" title="Add activity" value="Add activity" class="clearfix">
</form>

<form action="add_file.php?applicationid=$id" method="post" id="met-form">
	<input type="submit" title="Add file" value="Add file" class="clearfix">
</form>
END;

/* !Show the activities for this job application id. */
$query = "SELECT id, date, DATE_FORMAT(TIMESTAMP,'%H:%i%p') AS time, eventdetails FROM ".EVENT_TABLE." WHERE applicationid=$id";
$result = $database_object->query($query);
$i = 1;
