...
$query = "SELECT id, DATE_FORMAT(date, '%d/%m/%y') AS date_formated, role, reference, day_rate, salary_low, salary_high, via, company, contact_name, contact_email, url, agency, status, follow_up, last_name, first_name FROM ".APP_TABLE." ORDER BY date DESC LIMIT ".SUMMARY_NUMBER;
$result = $database_object->query($query);
...
/* !Only add the URL link if one exists in the database. */
	if($line['url']!=""){
		echo "<a href='".htmlspecialchars($line['url'])."' target='_blank'>".substr($line['role'],0,COL_WIDTH_ROLE)."</a>";
	} else {
	...
	echo "<td><a href='mailto:$line[contact_email],$edited_email?subject=Reference:%20".htmlspecialchars($line['reference'])."&amp;body=Dear%20$line[first_name]'>".substr($line['contact_email'],0,COL_WIDTH_EMAIL)."</a></td><td>$line[status]</td>\n";
	echo "</tr>\n\n";
}
